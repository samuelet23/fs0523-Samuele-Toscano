...
*/
{
  function somma (arr) {
    let sommaArray = 0
    let risultato = arr.reduce((Tot,Valore) => Tot + Valore, 0)
  return risultato 
  }
let arr = [12, 35, 49, 392, 1928, 28, 2, 38, 19, 28, 19, 12]
console.log(somma (arr));
...
function incremento (n, arr) {
 let array2 =arr.map ((numero) => numero * n )
return array2
}
let arr = [12, 35, 49, 392, 1928, 28, 2, 38, 19, 28, 19, 12]
console.log(incremento (2, arr ));
...
*/
{
  ...
  function stringhe(arr) {
  let risultato = arr.map ((str) => str.length)
return risultato

}
let arr = ['pollo', 'pizza', 'sjfj', 'ciao', 'ciao mondo']
console.log (arr);
console.log(stringhe(arr));
...
console.log( `il numero di film contenuti nell'array è ${film()}`);
...
{
function titoli () {
 let nomeTitoli= movies.map ((n) => n.Title )
 return nomeTitoli
}
console.log(titoli());
...
*/
{
function filmCorrenti () {
 let nuoviFilm= movies.filter ((n) => n.Year >2000 && n.Year < 3000)
 return nuoviFilm
}
console.log(filmCorrenti());
}
/* ESERCIZIO 13 (reduce)
